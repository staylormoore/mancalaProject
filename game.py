...
class Game:
    ...
    def determine_winner(self):  # determines which player wins by counting up the seeds in each pit once the
        ...
        p1seeds = 0
        p2seeds = 0
        for x in p1pits_dup - 1:  # iterates through p1pits, getting the # of seeds that may be leftover
            p1seeds += p1pits_dup[x]
        for x in p2pits_dup - 1:  # iterates through p2pits, getting the # of seeds that may be leftover
